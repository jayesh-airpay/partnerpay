...
class MerchantController extends Hcontroller
{
    ...
    public function actionCreate()
    {
        ...
        if(Yii::$app->request->post('MerchantMaster')['CREATE_QR']) {
            $model->CREATE_QR = 'Y';
        } else {
            $model->CREATE_QR = 'N';
        }
        ...
        if ($model->load(Yii::$app->request->post())) {
        	...
        	$model->B_LOGO = UploadedFile::getInstance($model, 'B_LOGO');
    ...
    /**
     * Deletes an existing MerchantMaster model (bank logo).
	...
	public function actionRemove($id)
    {
        ...
        $model->BANK_LOGO = '';
        if($model->update()) {
        	\Yii::$app->getSession()->setFlash('success', 'Bank logo removed successfully.');
            return $this->render('update', [
                'model' => $model,
            ]);
        }
    ...
    {
        return $this->hasOne(UserMaster::className(), ['MERCHANT_ID' => 'MERCHANT_ID']);
    }
}
