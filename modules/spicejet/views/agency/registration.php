...
<script type="text/javascript">
		...
		//validation loop
		 for (var i = 0; i <= card_type_count; i++) {
			...
			if(isValidate)
			{
				// call ajax
				$.ajax({
					url: "/partnerpay/web/spicejet/agent-registration/savedata",  
    ...
    $('.form3').hide();
		...
		$('body').on('click', '.radioBtn', function () {
			...
			if(value == 'Netbanking')
			{
				...
				$.ajax({
				url: "/partnerpay/web/spicejet/agent-registration/getbankdata",  
				type: "POST",
				dataType: "json",
				success: function(data) {
					if(data['status'])
	...
	function getPancardUnique(pan_card_value){
		var flag = false;
		$.ajax({
	url: "/partnerpay/web/spicejet/agent-registration/getpannumber",  
