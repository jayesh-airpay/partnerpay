...
//        if(true) {
            ...
            if(!empty($card)) {
                if(!empty($card['BANK_ID'])) {
                    if(!empty($group_limit_info)) {
                        if($group_limit_info['NETBANKING_LIMIT'] < $amount) {
                            $is_error = true;
                            $message['amount'] = "Minimum upload Amount for net banking is Rs. 10.00.";
                        }
                    }
                } else {
                    if(!empty($group_limit_info)) {
                        if($card['CARD_TYPE'] == 'CREDIT' && $group_limit_info['CREDIT_CARD_LOWER_LIMIT'] > $amount) {
                            $is_error = true;
                            $message['amount'] = "Amount is too low.";
                        }
                        if($card['CARD_TYPE'] == 'CREDIT' && $group_limit_info['CREDIT_CARD_UPPER_LIMIT'] < $amount) {
                            $is_error = true;
                            $message['amount'] = "Amount exceeded for credit card.";
                        }
                        if($card['CARD_TYPE'] == 'DEBIT' && $group_limit_info['DEBIT_CARD_LIMIT'] < $amount) {
                            $is_error = true;
                            $message['amount'] = "Amount exceeded for debit card.";
                        }
                        if($group_limit_info['OVERALL_GROUP_LINIT'] < $amount) {
                            $is_error = true;
                            $message['amount'] = "Amount exceeded for group.";
                        }
                    }
                }
            }
...
//        if(true) {
    ...
    public function actionSendtoairpay()
    {
        ...
        $create_invoice_query = "INSERT INTO tbl_invoice (PARTNER_ID, REF_ID, AMOUNT, TOTAL_AMOUNT, ISSUE_DATE, INVOICE_STATUS, CREATED_ON, CLIENT_EMAIL , CLIENT_MOBILE, AGENT_ID) VALUES(:partner_id, :ref_id, :amount, :total_amount, :issue_date, :invoice_status, :created_on, :client_email, :client_mobile, :agent_id)";
        ...
        $create_invoice->bindValue(':agent_id', $agent_details_id);
        ...
        if (empty($card['BANK_ID'])) {
            ...
            $post_data['channel'] = 'Nb';
            $post_data['chmod'] = 'Nb';
            $post_data['bankCode'] = $agent_bank['BANK_CODE'];
        }
        $url = 'https://payments.airpay.co.in/pay/directindexapi.php';
        return $this->renderPartial('../payment/sendtoairpay',
            [
                'post_data' => $post_data,
                'url' => $url,
            ]
...
//        if(true) {
  ...
  "result": {
    ...
    "token": null
            ...
            //var_dump($savecard_data);exit;
            //var_dump($save_card_api_response);exit;
            ...
            if (!empty($save_card_api_response['status']) && $save_card_api_response['status'] == '200') {
                ...
                $card_unique_code = $save_card_api_response['result']['carduniquecode'];
                $token = $save_card_api_response['result']['token'];
                //if(false) {
                $add_card_query = "INSERT INTO tbl_agent_payment_config(AGENT_DETAILS_ID, GROUP_ID, CARD_TYPE, CARD_NUMBER, NAME, EXPIRY_YEAR, EXPIRY_MONTH, TOKEN_NO, CARD_UNIQUE_CODE, STATUS, CREATED_ON, BANK_ID) VALUES (:agent_detail_id, :group_id, :card_type, :card_number, :name, :expiry_year, :expiry_month, :token, :card_unique_code, :status, :created_on, :bank_id)";
                ...
                $general_helper = new generalHelper();
                if (!empty($card_number)) {
                    $card_number = $general_helper->ccMasking($card_number);
                }
    ...
    public function actionAddbanks()
    {
        ...
        $connection = \Yii::$app->db;
        $get_agent_detais = $connection->createCommand('SELECT A.*,B.* FROM tbl_agent_details as A INNER JOIN tbl_partner_master as B ON A.PARTNER_ID = B.PARTNER_ID WHERE A.PARTNER_ID=:partner_id');
